 ...
 */
public class MessageFragmentReadNo extends BasicFragment {
    ...
    private void loadMessageData(int page, boolean isRefresh) {
        if (mListener != null) {
            ...
            if (mAdapter != null && !isRefresh) {
                ...
                mListener.onAction(BussinessConstants.SettingHttpErrorCode.MESSAGE_FRAGMENT_READ_NO_SHOW_LOADING_ACTION_ID, parame);
            } else {
                mListener.onAction(BussinessConstants.SettingHttpErrorCode.MESSAGE_FRAGMENT_READ_NO_ACTION_ID, parame);
            }
        }
    }
