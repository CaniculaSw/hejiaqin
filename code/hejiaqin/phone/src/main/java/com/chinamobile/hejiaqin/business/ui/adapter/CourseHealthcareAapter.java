...
public class CourseHealthcareAapter extends RecyclerView.Adapter {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (mData.get(position) != null) {
            ...
            textViewHolder.tv_title2.setText(StringUtil.isNullOrEmpty(practiceInfo.getTitle()) ? "" : practiceInfo.getTitle());
            if (practiceInfo.getRange() != null && practiceInfo.getRange().get(position) != null) {
                textViewHolder.tv_position.setText(StringUtil.isNullOrEmpty(practiceInfo.getRange().get(position).getCaption()) ? "" : mContext.getResources().getString(R.string.course_exercise_body_position) + practiceInfo.getRange().get(position).getCaption());
            }
            if (practiceInfo.getApparatus() != null && practiceInfo.getApparatus().get(position) != null) {
                textViewHolder.tv_equipment.setText(StringUtil.isNullOrEmpty(practiceInfo.getApparatus().get(position).getCaption()) ? mContext.getResources().getString
                        (R.string.course_exercise_equipment) + mContext.getResources().getString(R.string.course_exercise_equipment_non) : practiceInfo.getApparatus().get(position).getCaption());
            }
            textViewHolder.tv_brocast_times.setText(practiceInfo.getParticipants() + mContext.getResources().getString(R.string.course_forum_brocast_num));
            boolean hasJoined = practiceInfo.getJoined() == 1;
            if (!hasJoined) {
                textViewHolder.imag_tag.setVisibility(View.INVISIBLE);
            }

            String img_url = StringUtil.isNullOrEmpty(practiceInfo.getCoverImage()) ? "-" : practiceInfo.getCoverImage();
            Picasso.with(mContext).load(img_url).placeholder(R.mipmap.ic_default1).into(textViewHolder.imag_pic);
        }
    }
    ...
    public class TextViewHolder extends RecyclerView.ViewHolder {
        ...
        private TextView tv_brocast_times;
        ...
        public TextViewHolder(View view) {
            ...
            tv_brocast_times = (TextView) view.findViewById(R.id.textView6);
