...
public class UserInfoCacheManager {
    ...
    private static void saveUserSettingToLocal(Context context, TvSettingInfo setting) {
        HashMap map = new HashMap();
        Gson gson = new Gson();
        map.put(getUserInfo(context).getTvAccount() + BussinessConstants.Setting.USER_SETTING_KEY,
                gson.toJson(setting));
    ...
    public static void saveUserToMem(Context context, UserInfo info, long tokenDate) {
        if (info != null) {
            StorageMgr.getInstance().getMemStorage()
                    .save(BussinessConstants.Login.USER_INFO_KEY, info);
        }
        StorageMgr.getInstance().getMemStorage()
                .save(BussinessConstants.Login.TOKEN_DATE, tokenDate);
    }
    ...
    public static void saveBindDeviceToMem(Context context, UserList userList) {
        if (userList != null) {
            StorageMgr.getInstance().getMemStorage()
                    .save(BussinessConstants.Setting.BINDED_DEVICE_KEY, userList);
        }
    }
    ...
    public static void saveBindAppToMem(Context context, UserList userList) {
        if (userList != null) {
            StorageMgr.getInstance().getMemStorage()
                    .save(BussinessConstants.Setting.BINDED_APP_KEY, userList);
        }
    }
    ...
    public static boolean isBindedDevice(Context context, String num) {
        ...
        while (iterator.hasNext()) {
            UserInfo userInfo = iterator.next();
            if (CommonUtils.getPhoneNumber(num).equals(
                    CommonUtils.getPhoneNumber(userInfo.getTvAccount()))) {
                flag = true;
                break;
            }
        }
    ...
    public static VersionInfo getVersionInfo(Context context) {
        String infoStr = StorageMgr.getInstance().getSharedPStorage(context)
                .getString(BussinessConstants.Setting.VERSION_INFO_KEY);
        if (infoStr != null) {
            Gson gson = new Gson();
    ...
    public static TvSettingInfo getUserSettingInfo(Context context) {
        String settingStr = StorageMgr
                .getInstance()
                .getSharedPStorage(context)
                .getString(
                        getUserInfo(context).getTvAccount()
                                + BussinessConstants.Setting.USER_SETTING_KEY);
        if (settingStr != null) {
            Gson gson = new Gson();
    ...
    public static String getTvAccount(Context context) {
        String account = (String) StorageMgr.getInstance().getSharedPStorage(context)
                .getString(BussinessConstants.Login.TV_ACCOUNT_KEY);
    ...
    public static void saveVoipLogined(Context context) {
        StorageMgr.getInstance().getSharedPStorage(context)
                .save(BussinessConstants.Login.VOIP_LOGINED_KEY, true);
    }

    /***/
    public static void saveTvLogout(Context context) {
        StorageMgr.getInstance().getSharedPStorage(context)
                .save(BussinessConstants.Login.TV_IS_LOGOUT_KEY, true);
    }
    ...
    public static void saveSTBConfig(Context context, String userid, String token,
                                     String softVersion) {
        StorageMgr.getInstance().getSharedPStorage(context)
                .save(BussinessConstants.Login.TV_USERID_KEY, userid);
        StorageMgr.getInstance().getSharedPStorage(context)
                .save(BussinessConstants.Login.TV_TOKEN_KEY, token);
        StorageMgr.getInstance().getSharedPStorage(context)
                .save(BussinessConstants.Login.TV_SOFT_WARE_KEY, softVersion);
    }
    ...
    public static boolean getVoipLogined(Context context) {
        return StorageMgr.getInstance().getSharedPStorage(context)
                .getBoolean(BussinessConstants.Login.VOIP_LOGINED_KEY);
    }
    ...
    public static boolean getTvIsLogout(Context context) {
        return StorageMgr.getInstance().getSharedPStorage(context)
                .getBoolean(BussinessConstants.Login.TV_IS_LOGOUT_KEY);
    }

    public static String getTvUserID(Context context) {
        return StorageMgr.getInstance().getSharedPStorage(context)
                .getString(BussinessConstants.Login.TV_USERID_KEY);
    }

    public static String getTvToken(Context context) {
        return StorageMgr.getInstance().getSharedPStorage(context)
                .getString(BussinessConstants.Login.TV_TOKEN_KEY);
    }

    public static String getSoftware(Context context) {
        return StorageMgr.getInstance().getSharedPStorage(context)
                .getString(BussinessConstants.Login.TV_SOFT_WARE_KEY);
    }
}
