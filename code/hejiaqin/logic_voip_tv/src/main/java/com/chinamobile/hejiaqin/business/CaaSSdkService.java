...
public class CaaSSdkService {
    ...
    public static void setLocalCamaraStatus(boolean status) {
        LogUtil.d(Const.TAG_CAAS, "Enter setLocalCamaraStatus status:" + status + " old status:"
                + m_localViewOpen);
    ...
    public static void setLocalRenderPos(Rect rectLocal, int layer) {
        if (rectLocal != null) {
            LogUtil.d(
                    Const.TAG_CAAS,
                    "Enter setLocalRenderPos layer:" + layer + "rectLocal.width():"
                            + rectLocal.width() + "rectLocal.height()" + rectLocal.height()
                            + "rectLocal.left" + rectLocal.left + "rectLocal.top" + rectLocal.top);
            if (rectLocal.width() > 0 && rectLocal.height() > 0) {
                CallApi.setRegion(CallApi.VIDEO_TYPE_LOCAL, rectLocal.left, rectLocal.top,
                        rectLocal.width(), rectLocal.height(), layer);
            }
        }
    }
    ...
    public static void setRemoteRenderPos(Rect rectRemote, int layer) {
        ...
        if (rectRemote != null && rectRemote.width() > 0 && rectRemote.height() > 0) {
            CallApi.setRegion(CallApi.VIDEO_TYPE_REMOTE, rectRemote.left, rectRemote.top,
                    rectRemote.width(), rectRemote.height(), layer);
        }
    }
