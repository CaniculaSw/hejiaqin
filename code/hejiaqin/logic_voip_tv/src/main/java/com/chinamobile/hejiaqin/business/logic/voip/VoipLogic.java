...
public class VoipLogic extends LogicImp implements IVoipLogic {
            ...
            if (callSession.getType() == CallSession.TYPE_VIDEO) {
                ...
                if (isTv()) {
                    if(callSession.isNurse())
                    {
                        ...
                        VoipLogic.this.sendMessageDelayed(BussinessConstants.DialMsgID.CALL_ON_TV_INCOMING_MSG_ID, callSession.getSessionId(),200);
                    }
                } else {
    ...
    public CallSession call(String calleeNumber, boolean isVideoCall,boolean isPhoneAPP) {
        ...
        if (isVideoCall) {
            callSession = CallApi.initiateVideoCall(CommonUtils.getCountryPhoneNumber(CommonUtils.getTvNumber(CommonUtils.getPhoneNumber(tmpNumber))));
        } else {
            callSession = CallApi.initiateAudioCall(CommonUtils.getCountryPhoneNumber(CommonUtils.getTvNumber(CommonUtils.getPhoneNumber(tmpNumber))));
        }
        ...
        String recordId = UUID.randomUUID().toString();
        callRecord.setRecordId(recordId);
        callRecord.setPeerNumber(tmpNumber);
        callRecord.setNoCountryNumber(CommonUtils.getPhoneNumber(tmpNumber));
        callRecord.setBeginTime(DateTimeUtil.getDateString(new Date()));
        callRecord.setDuration(callSession.getDuration());
